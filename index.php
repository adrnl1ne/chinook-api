...
$router->addRoute('GET', '%/albums$%', function() {
    ...
    if (isset($_GET['s'])) {
        $search = $_GET['s'];
        return $controller->getAlbumBySearch($search);
    } else {
        return $controller->getAll();
    }
});
...
$router->addRoute('GET', '%/tracks$%', function() {
    ...
    if (isset($_GET['s'])) {
        ...
        return $controller->getTracksByComposer($composer);
    } else {
        return $controller->getAll();
    }
});
...
$router->addRoute('GET', '%/playlists$%', function() {
    ...
    if (isset($_GET['s'])) {
        $search = $_GET['s'];
        return $controller->getPlaylistBySearch($search);
    } else {
        return $controller->getAll();
    }
});
...
// Debug: Log method, URI, and endpoint
